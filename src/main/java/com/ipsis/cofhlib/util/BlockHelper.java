...
public final class BlockHelper {
	...
	public static TileEntity getAdjacentTileEntity(World world, int x, int y,
		...
		return world == null ? null : world.getTileEntity(x + dir.offsetX, y
	...
	public static TileEntity getAdjacentTileEntity(TileEntity refTile,
		...
		return refTile == null ? null : getAdjacentTileEntity(
				refTile.getWorldObj(), refTile.xCoord, refTile.yCoord,
				refTile.zCoord, dir);
	}
	...
	public static int rotateVanillaBlock(World world, Block block, int x,
			int y, int z) {

		int bId = Block.getIdFromBlock(block), bMeta = world.getBlockMetadata(
				x, y, z);
		...
		case RotationType.CHEST:
			...
			for (int i = 2; i < 6; i++) {
				...
				if (isEqual(world.getBlock(coords[0], coords[1], coords[2]),
						block)) {
	...
	public static int rotateVanillaBlockAlt(World world, Block block, int x,
			int y, int z) {
	...
	public static List<ItemStack> breakBlock(World worldObj, int x, int y,
			int z, Block block, int fortune, boolean doBreak, boolean silkTouch) {
		...
		List<EntityItem> result = worldObj.getEntitiesWithinAABB(
				EntityItem.class, AxisAlignedBB.getBoundingBox(x - 2, y - 2,
						z - 2, x + 3, y + 3, z + 3));
